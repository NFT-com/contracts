  ...
  try {
    ...
    describe("LP Staking Yield Contract", async function () {
      it("should allow owner to create contract and users to start earning yield", async function () {
        ...
        const wethNftLpPair = await deployedUniV2Factory.getPair(TESTNET_WETH, deployedNftToken.address);
        console.log("wethNftLpPair: ", wethNftLpPair);
        const xeenusNftLpPair = await deployedUniV2Factory.getPair(TESTNET_XEENUS, deployedNftToken.address);
