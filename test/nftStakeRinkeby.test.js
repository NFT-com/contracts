...
describe("NFT Token Staking (Rinkeby)", function () {
  try {
    ...
    it("should allow a user to test swap XEENUS (or arbitrary ERC20) for NFT coins on Uniswap V3", async function () {
      ...
      await deployedXEENUS
        .connect(owner)
        .transfer(deployedNftStake.address, BigNumber.from(10).pow(BigNumber.from(20)));

      expect(await deployedXEENUS.balanceOf(deployedNftStake.address)).to.be.equal(
        BigNumber.from(10).pow(BigNumber.from(20)),
      );
