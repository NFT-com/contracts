...
describe("ERC721WithContractWideRoyalties", () => {
  ...
  beforeEach(async () => {
    ...
    ERC721WithContractWideRoyalties = await ethers.getContractFactory("ERC721WithContractWideRoyalties");
  ...
  describe("Contract wide Royalties", async () => {
    ...
    it("throws if royalties more than 100%", async function () {
      const tx = erc721WithRoyalties.setRoyalties(royaltiesRecipient.address, 10001);
      await expect(tx).to.be.revertedWith("ERC2981Royalties: Too high");
    });

    it("has the right royalties for tokenId", async function () {
      await erc721WithRoyalties.setRoyalties(royaltiesRecipient.address, 250);
