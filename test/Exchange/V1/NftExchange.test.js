...
describe("NFT.com Exchange", function () {
  try {
    ...
    describe("Allow simple Swaps", function () {
      it("should allow EOA users to make bids using sigV4", async function () {
        ...
        let makeAsset = await getAssetHash(
          ERC721_ASSET_CLASS,
          encode(["address", "uint256"], [NFT_PROFILE_RINKEBY, 0]),                               // bytes encoding of contract and token id
          1                                                                                       // 1 quantity of 721
        );
        ...
        let takeAsset = await getAssetHash(
          ERC20_ASSET_CLASS,
          encode(["address"], [NFT_RINKEBY_ADDRESS]),                                             // bytes encoding of contract
        ...
        let start = 0;
        let end = 0;
        let data = encode(["uint256"], [minimumBidValue]);
        ...
        const exchangeOrderDigest = await getDigest(
          ...
          getHash(
            ["bytes32", "address", "bytes32", "address", "bytes32", "uint256", "uint256", "uint256", "bytes"],
            [EXCHANGE_ORDER_TYPEHASH, maker, makeAsset, taker, takeAsset, salt, start, end, data],
          ),
        );

        const { v: v0, r: r0, s: s0 } = sign(exchangeOrderDigest, ownerSigner);
